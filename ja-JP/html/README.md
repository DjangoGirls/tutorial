...
`<your-pythonanywhere-domain>`を、山カッコ<>なしで実際のPythonAnywhereのユーザー名に置き換える必要があります。 通常、サブドメイン名はPythonAnywhereユーザー名ですが、場合によっては多少異なることがあります（ユーザー名に大文字が含まれている場合など）。 このコマンドが機能しない場合は、`ls`（ファイルを一覧にする）コマンドを使用して、サブドメイン名を含んだ実際のフォルダ名を見つけ、そこに`cd`します。
