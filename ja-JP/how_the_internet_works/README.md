...
まず最初に理解する必要があるのは、Webサイトはハードディスクに保存された沢山のファイルであるということです（ちょうどあなたの動画や音楽、画像のデータのように）。 ただし、Webサイトに固有の部分が1つあります。HTMLと呼ばれるコンピューターコードが含まれていることです。

プログラミングに慣れていない場合、最初はHTMLを把握するのは難しいかもしれませんが、Webブラウザー（Chrome、Safari、Firefoxなど）とHTMLコードは親和性があります。 Webブラウザはこのコードがわかるようになっていて、コードの指示に従います。そして、あなたのウェブサイトのファイルをあなたが望む方法で表示するのです。
