...
OS X と Linux には、`man` コマンドがあり、それはコマンドのヘルプページを提供します。 `man pwd` を試してみましょう。他のコマンドも、コマンドの前に `man` を置くことでヘルプを見れるので試してみましょう。 `man` の出力は、通常はページングされています。 次のページに移動するにはスペースキーを使います。 ヘルプを閉じるには `q` を押します。
