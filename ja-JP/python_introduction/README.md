...
ここであなたは、最後の行になぜ関数の名前を書いたのだろう、と疑問に感じたかもしれません。 `def hi():` とそれに続くインデントされた数行を書くと、これは `hi()` という関数が何をすべきかという命令を書いたことになります。 Pythonはこれらの命令を読み込み記憶しますが、関数はまだ実行されません。 関数を実行したいとPythonに伝えるには、 `hi()` として関数を呼び出す必要があります。 Pythonはファイルを読み込み、先頭から末尾へと実行するので、関数を呼び出す前に定義しなければならないのです。
