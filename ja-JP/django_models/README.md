...
さて、ブログの投稿（記事、post ポスト）をためておくものが欲しいですよね。そのために `オブジェクト` についてちょっとお話しします。
...
ではブログの投稿でどんなことができたらいいでしょうか？投稿が公開されるといいですよね？
...
オブジェクトが何か分かったので、ブログの投稿のDjangoモデルを作りましょう。

Djangoのモデルは特別なオブジェクトで、`データベース` に格納されます。 データベースはデータの集まりです。 ここにユーザーやブログの投稿の情報を格納します。 データを格納するのにSQLiteデータベースを使います。 これはDjangoのデフォルトのデータベースで、今はこれで十分です。
...
`blog/models.py` ファイルで `Model` と呼ばれるオブジェクトを全て定義します。これがブログの投稿を定義する場所です。
...
- `models.Model` はPostがDjango Modelだという意味で、Djangoが、これはデータベースに保存すべきものだと分かるようにしています。
...
- `models.TextField` – これは制限無しの長いテキスト用です。ブログの投稿のコンテンツに理想的なフィールドでしょ？
...
メソッドは通常何かを `return` します。 一つの例が `__str__` メソッドにあります。 このシナリオでは、`__str__()` を呼ぶと、投稿のタイトルのテキスト（**string**）が返ってきます。
...
やった～！Postモデルがデータベースに入りました。どうなったか見たいでしょ？次へ進みましょう！