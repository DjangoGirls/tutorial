...
برای ایجاد یک فرم جدید`Post`، ما باید `PostForm()` را فراخوانی کنیم و آن را به قالب منتقل کنیم. ما به نمای *view* باز خواهیم گشت، اما در حال حاضر، بیایید به سرعت یک قالب برای فرم ایجاد کنیم.

## قالب

لازم است یک فایل به نام `post_edit.html`در دایرکتوری `blog/templates/blog`بسازیم و آن را در ویرایشگر کد بازکنیم. برای آنکه یک فرم کار کند به چند چیز نیاز داریم:
...
جواب این است: هیچ چیز. ما باید در نمای *view* کمی کارهای بیشتر انجام دهیم.
...
هنگامی که فرم را ارسال می کنیم، ما به همان نمای قبل باز میگردیم، اما این بار ما در `request` داده‌های بیشتری داریم، به ویژه در `request.POST` (نامگذاری request.POST هیچ ارتباطی با "پست" وبلاگ ندارد و به معنی آن است که ما در حال ارسال اطلاعات هستیم). به یاد می آورید که چگونه در فایل HTML، تعریف `<form>` دارای متغیر `method="POST"` بود؟ تمام فیلدهای فرم اکنون در `request.POST` قرار دارند. شما نباید عبارت `POST` را در اینجا به چیز دیگری تغییر دهید (تنها مقدار معتبر دیگر برای `method` مقدار `GET` است، اما الان وقت آن را نداریم تا تفاوت آن‌ها را توضیح دهیم).

بنابراین در نمای *view* ما دو موقعیت جداگانه برای رسیدگی داریم: اول اینکه زمانی که ما برای اولین بار به صفحه می رویم و یک فرم خالی را می خواهیم، و دوم، وقتی برگشت به نمای *view* با تمام داده های فرم که ما تایپ کرده ایم. بنابراین ما نیاز به اضافه کردن یک شرط داریم (ما از `if`برای این منظور استفاده می‌کنیم):
...
`post_detail` نام نمایی است که ما می‌خواهیم به آن برویم. به یاد داشته دارید که این نمای *view* نیاز به متغیر `pk` دارد؟ برای انتقال آن به نمایش ها، از `pk=post.pk` استفاده می‌کنیم، که `post` جدیدترین پست وبلاگ است!

خوب، ما خیلی صحبت کرده‌ایم، اما احتمالا می‌خواهیم ببینیم که کل *view* چگونه به نظر می‌رسد، درست است؟
...
ممکن است متوجه شده باشید که قبل از ذخیره پست، تاریخ انتشار را تنظیم کرده‌ایم. بعدها، ما دکمه *publish button* را در بخش ** آموزش دختران جنگو: برنامه های افزودنی ** معرفی می‌کنیم.

این عالیه!

> همانطور که اخیراً رابط کاربری مدیر جنگجو را استفاده کردیم، سیستم در حال حاضر فکر می‌کند ما هنوز درون رابط کاربری هستیم. چند موقعیت وجود دارد که می تواند منجر به خروج ما از رابط کاربری مدیریت شود (بسته شدن مرورگر، راه اندازی مجدد DB و غیره). اگر در هنگام ایجاد یک پست متوجه شدید که خطایی در رابطه با عدم وجود یک کاربر ایجاد شده است، به صفحه مدیریت http://127.0.0.1:8000/admin بروید و دوباره وارد شوید. این مسئله به طور موقت حل خواهد شد. یک ثابت دائمی در انتظار شما در ** Homework: اضافه کردن امنیت به وب سایت شما! </ 0> فصل بعد از آموزش اصلی وجود دارد.</p> </blockquote> 
> 
> ![خطا ورود به سیستم](images/post_create_error.png)
> 
> ## اعتبار فرم
> 
> در حال حاضر، ما به شما نشان می دهیم که چگونه شکل های جانسون سرد است. یک پست وبلاگ باید زمینه ` عنوان </ 0> و <code> متن </ 0> داشته باشد. در مدل <code> پست </ 0> ما نگفتیم که این فیلدها (به غیر از <code> published_date </ 0>) مورد نیاز نیستند، بنابراین، دیوانه، به طور پیش فرض، انتظار می رود آنها را تنظیم کند.</p>
