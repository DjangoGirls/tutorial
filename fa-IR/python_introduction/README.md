...
If you want to exit the Python console at any point, type `exit()` or use the shortcut `Ctrl + Z` for Windows and `Ctrl + D` for Mac/Linux. پس از خروج `>>>` را دیگر نمی‌بینید.
