...
زمانی که فقط `name` را تایپ می‌کنید، مفسر پایتون با *ارائه‌ای* از رشته متنی داخل متغیر 'name' پاسخ می‌دهد، که این پاسخ دنباله کاراکترهای M-a-h-s-a درون علامت نقل قول است. وقتی که `print(name)` را اجرا می‌کنید، پایتون محتوای متغیر را در صفحه نمایش چاپ می‌کند، البته بدون علامت‌های نقل قول که مرتب‌تر است.

بعداً خواهیم دید که `print()` زمانی که می‌خواهیم از داخل توابع چیزی را چاپ کنیم، یا زمانی که می‌خواهیم چندین خط از متن را چاپ کنیم، نیز مفید است.
...
وقتی آن را اجرا کنید، خروجی زیر را چاپ می‌کند:
...
فکر میکنید اگر نام دیگری را بنویسید چه اتفاقی می‌افتد؟(بغیر از Ola یا Sonja). امتحان کنید و ببینید که درست فکر می کردید یا نه. باید این را چاپ کند:
...
Note that the second of these two numbers is not included in the list that is output by Python (meaning `range(1, 6)` counts from 1 to 5, but does not include the number 6). به دلیل این که "range" نیمه باز است، و منظور این است که شامل عدد ابتدای بازه می شود ولی عدد انتهایی را شامل نمی شود.
