...
Sedaj že veste, da mora biti spletna stran na strežniku. Obstaja veliko ponudnikov spletnega gostovanja. We will use one that has a relatively simple deployment process: [PythonAnywhere](https://www.pythonanywhere.com/). PythonAnywhere je brezplačno gostovanje za manjše aplikacije, ki nimajo velikega števila obiskovalcev, kar zadostuje našim trenutnim potrebam.

The other external service we'll be using is [GitHub](https://www.github.com), which is a code hosting service. Seveda so na voljo tudi druge rešitve, a večina programerjev ima GitHub račun. In sedaj ga imate tudi vi!
...
> **Opomba** Ne pozabite pike na začetku imena, saj je pomembna! If you're having any difficulty creating it (Macs don't like you to create files that begin with a dot via the Finder, for example), then use the "Save As" feature in your editor; it's bulletproof.
> 
...
- [new branch] master -> master Branch master set up to track remote branch master from origin.
    ...
    Your code is now on GitHub. Pojdite na spletno stran in se prepričajte!  You'll find it's in fine company – [Django](https://github.com/django/django), the [Django Girls Tutorial](https://github.com/DjangoGirls/tutorial), and many other great open source software projects also host their code on GitHub. :)
    ...
    Prenesimo kodo iz spletnega mesta GitHub, tako da ustvarimo "klon" repozitorija. Type the following into the console on PythonAnywhere (don't forget to use your GitHub username in place of `&lt;your-github-username&gt;`):
...
The `StaticFilesHandler` is for dealing with our CSS. This is taken care of automatically for you during local development by the `runserver` command. S statičnimi datotekami se bomo podrobneje seznanili v nadaljevanju, ko bomo urejali CSS datoteko za našo stran.
...
Če opazite napako, ko greste na vašo spletno stran, je prvi naslov za razhroščevanje **error log**. Povezavo najdete v zavihku [Web](https://www.pythonanywhere.com/web_app_setup/) na PythonAnywhere. Preverite, ali je zabeleženo kakšno sporočilo o napakah; novejše so na dnu. Pogoste napake so:
...
The default page for your site should say "It worked!", just like it does on your local computer. Poizkusite dodati `/admin/` na koncu URL-ja in prikazati se morala admin stran. Prijavite se z uporabniškim imenom in geslom. Sedaj lahko dodajate nove objave na strežniku.
