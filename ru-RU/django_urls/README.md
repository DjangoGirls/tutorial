...
Вперёд, добавь строку для импорта `blog.urls`. Вам также нужно будет изменить строки `from django.urls...`, потому что мы будем использовать функцию `include`, так что вам нужно добавить этот import в строку.
