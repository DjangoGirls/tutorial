...
`post_detail` is the name of the view we want to go to. Lembre-se que essa *view* exige uma variável `pk`? To pass it to the views, we use `pk=post.pk`, where `post` is the newly created blog post!
...
Ser capaz de criar novos posts apenas clicando em um link é ótimo! But right now, anyone who visits your site will be able to make a new blog post, and that's probably not something you want. Let's make it so the button shows up for you but not for anyone else.
