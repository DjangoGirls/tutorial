...
Uma *view* é colocada onde nós colocamos a "lógica" da nossa aplicação. Ela irá solicitar informações a partir do `model` que você criou antes e passá-la para um `template` que você vai criar no próximo capítulo. Views, no fundo, não passam de métodos escritos em Python que são um pouco mais complicados do que aquilo que fizemos no capítulo **Introdução ao Python**.
