...
Jak widać, stworzyłyśmy funkcję (`def`) nazwaną `post_list`, która pobiera `request` i zwraca `return` wartość uzyskana dzięki wywołaniu innej funkcji `render`, która wyrenderuje (złoży w całość) nasz szablon `blog/post_list.html`.
