...
So in our *view* we have two separate situations to handle: first, when we access the page for the first time and we want a blank form, and second, when we go back to the *view* with all form data we just typed. Da bi naša funkcija pravilno razrešila obe situacijo moramo dodati pogoj (uporabili bomo `if`):
