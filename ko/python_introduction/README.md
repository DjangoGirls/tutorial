...
파이썬과 놀기 위해서는 *커맨드라인*을 열어야 합니다. [커맨드라인 시작하기][1]에서 배웠던 걸 기억하고 있겠지요?

 [1]: ../intro_to_command_line/README.md

준비되었다면, 아래 내용을 잘 따라오세요.

파이썬 콘솔을 실행해야하려면, 윈도우에서는 `python`을 입력하고 맥OS나 리눅스라면`python3`를 입력하고 `엔터`를 입력하세요.
...
잘했어요! 값이 출력되었나요? 파이썬은 수학을 할 줄 알아요. 다른 명령도 시험해봐요: - `4 * 5` - `5 - 1` - `40 / 2`
