...
Python과 함께 놀기 위해서는 *커맨드라인*을 열어야 합니다. [커맨드라인 시작하기][1]에서 배웠던 걸 기억하고 있겠지요?
...
*   **프롬프트** - 명령어(코드) 를 파이썬 프롬프트에 입력하면 파이썬이 응답합니다.
*   **숫자와 문자열** - 파이썬에서는 셈을 할 때는 숫자를 사용하고 글을 다룰 때는 문자열을 사용합니다.
*   **연산자** - 값들을 이용해서 새로운 값을 만들 때는 + 나 * 같은 연산자를 이용합니다.
*   **함수** - upper() 나 len () 처럼 대상에 명령을 수행합니다.
