...
`blog/urls.py`파일에 URL을 만들어, 장고가 `post_detail`이란 *view<1/>로 보내, 전체 블로그 글이 보일 수 있게 만들어 봅시다. Add the line `url(r'^post/(?P<pk>\d+)/$', views.post_detail, name='post_detail'),` to the `blog/urls.py` file. 그러면 아래처럼 보일 거에요. :</p> 
