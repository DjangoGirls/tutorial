...
Es ist an der Zeit, `blog/templates/blog/base.html` im Code-Editor zu öffnen. Now we can use this icon file inside the base template as follows. Im `div`-Element innerhalb des `header`-Abschnitts werden wir einen Link vor dem `h1`-Element hinzufügen:
