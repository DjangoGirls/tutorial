...
Вставь в окно терминала команду `git clone` и ссылку, которую ты скопировала:

     $ git clone <скопированная_ссылка>

Дописывать угловые скобки не надо!
...
Так же как ты делала на своем компьютере, ты можешь создать виртуальное окружение на PythonAnywhere. В Bash консоли введи следующую команду:
    ...
    (myvenv) $  pip install django==1.8 whitenoise
    Collecting django
    [...]
...
Задаешься вопросом: «что это за whitenoise такой»? Это утилита для работы с так называемыми «статическими файлами». Статические файлы — это файлы, не содержащие программного кода, например, файлы HTML или CSS. Статические файлы работают на серверах не так, как на твоём локальном компьютере, и нам потребуется инструмент, такой как "whitenoise", чтобы управляться с ними.
...
> **Примечание:** не забудь заменить имя пользователя на своё там, где указано `<your-username>`
