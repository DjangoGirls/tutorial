...
Ты только что использовала **функцию** `upper` для своей строки! Функция (такая как `upper()`) представляет собой набор инструкций, который должен выполнить Python над заданным объектом (в нашем случае: `"Ola"`) при её вызове.
...
Так же как в примере со списками, использование функции `len()` вернет число пар ключ/значение в словаре. Попробуй сама:
...
*   **and** — если ты используешь оператор `and`, оба сравнения по бокам от него должны быть True (верны), чтобы результат всей команды был равен True.
*   **or** — если ты используешь оператор `or`, достаточно одному из сравнений по бокам от него быть равным True, чтобы результат всей команды также равнялся True.
...
*   **сравнением вещей** — в Python ты можешь сравнивать вещи друг с другом при помощи `>`, `>=`, `==`, `<=`, `<` и операторов `and`, `or`;
