...
È come mettere le cose in delle scatole. You have one big box, `<html></html>`; inside it there is `<body></body>`, and that contains still smaller boxes: `<p></p>`.
...
> **Note** `--all` means that `git` will also recognize if you've deleted files (by default, it only recognizes new/modified files). Ricorda anche (dal capitolo 3) che il `.` significa la directory attuale.
