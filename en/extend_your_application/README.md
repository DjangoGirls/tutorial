...
We will start with adding a link inside `blog/templates/blog/post_list.html` file. Open it in the text editor, and so far it should look like this:
...
Let's make a URL in the `blog/urls.py` file to point Django to a *view* named `post_detail`, that will show an entire blog post. Open the `blog/urls.py` file in the text editor, and add the line `path('post/<int:pk>)/', views.post_detail, name='post_detail'),` so that the file looks like this:
...
We should open `blog/views.py` in the text editor and add the following code near the other `from` lines:
