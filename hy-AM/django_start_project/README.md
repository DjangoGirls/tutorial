...
> 
> Եթե ​​դուք վարում եք ձեր նախագիծը `Glitch.com`եկեք պաշտպանենք Django- ի գաղտնի բանալին, որն անհրաժեշտ է գաղտնիության ապահովման համար (հակառակ դեպքում ձեր նախագիծը վերամշակող ցանկացած անձ կարող է տեսնել այն). * Նախ, մենք պատրաստվում ենք ստեղծել պատահական գաղտնի բանալի: Կրկին բացեք Glitch տերմինալը և մուտքագրեք հետևյալ հրահանգը. {% filename %}.env{% endfilename %} 
> 
>     bash
>         python -c 'from django.core.management.utils import get_random_secret_key; \
>               print(get_random_secret_key())' Սա պետք է ցուցադրի երկար պատահական տող, որը կատարյալ է որպես գաղտնի բանալի ՝ ձեր բոլորովին նոր Django կայքի համար: Այժմ մենք կտեղադրենք այս բանալին a- ի մեջ 
> 
> `.env` ֆայլ, որը Glitch- ը ցույց կտա ձեզ միայն այն դեպքում, եթե դուք եք կայքի սեփականատերն եք:  
>   
> * Ստեղծել ֆայլ `.env` ձեր ծրագրի հիմքում և դրանում ավելացրեք հետևյալ հատկությունը. {% filename %}.env{% endfilename %} 
> 
>     bash
>         # Here, inside the single quotes, you can cut and paste the random key generated above
>         SECRET='3!0k#7ds5mp^-x$lqs2%le6v97h#@xopab&oj5y7d=hxe511jl' * Դրանից հետո թարմացրեք Django- ի կարգավորումների ֆայլը ՝ այս գաղտնի արժեքը մուտքագրելու համար և տեղադրեք Django կայքի անվանումը ՝ {% filename %}mysite/settings.py{% endfilename %} 
> 
>     python
>         SECRET_KEY = os.getenv('SECRET') * Եվ մի փոքր հեռու, նույն ֆայլում, մենք մուտքագրում ենք ձեր նոր Glitch կայքի անունը. 
> 
>     python
>         ALLOWED_HOSTS = [os.getenv('PROJECT_DOMAIN') + ".glitch.me"] The 
> 
> `PROJECT_DOMAIN` արժեքը ավտոմատ կերպով առաջացնում է Glitch- ը: Այն կհամապատասխանի ձեր ծրագրի անվանը:
